 ...
 * {@link TypeName} always, with utilities to convert it back to {@link
 * Element} and {@link TypeMirror}.
 *
...
abstract class GeneralInjectorGenerator {
  ...
  protected final static String LOCK_HOLDER_PACKAGE_STRING = "lock.holder";
  protected final static String LOCK_HOLDER_CLASS_STRING = "LockHolder";
  protected final static String LOCK_HOLDER_FIELD_STRIN = "theLock";
  ...
  protected final Set<TypeElement> componentDependencies;
  ...
  protected MethodSpec.Builder initializeBuilder;
  ...
  protected final Set<TypeElement> modules;
  ...
  protected abstract void preGenerateProduced();
  // Do implementation specific stuff.
  protected abstract void postGenerateProduced();
  ...
  private void generateProduced() {
    logger.w("provision count: %d", getProduced().getFirst().size());
    logger.w("injection count: %d", getProduced().getSecond().size());
  ...
  private void generateCtor() {
    ...
    if (!Sets.newHashSet(
            ...
            "DaggerRootApplicationComponent",
            "DaggerPlatformApplicationComponentImpl")
        .contains(getInjectorSimpleName())) {
      // ctorBuilder.addStatement("$L()", INITIALIZE_METHOD_NAME);
    }
    injectorBuilder.addMethod(ctorBuilder.build());
  }

  protected final static String getPackageFromInjectorClassName(ClassName injectorClassName) {
   ...
   */
  protected final ClassName getInjectorNameFor(BindingKey key, TypeElement referencingClass) {
  ...
  protected final ClassName getPackagedInjectorNameForDependencyInfo(
  ...
  protected final void generateProvisionMethodIfNeeded(BindingKey key) {
    ...
    if (utils.isOptional(key)
        ...
        case UNIQUE:
          switch (dependencyInfo.getDependencySourceType()) {
            ...
            case EITHER_COMPONENT:
              generateProvisionMethodForEitherComponent(key);
              break;
      ...
      logger.w("really!? we have handling generic: %s", key);
      logger.n("stack:");
      ...
      if (genericDependencyInfo != null) {
        ...
        utils.generateDebugInfoMethod(
            ...
            "diNotFound",
            String.format(
                "(sub)component: %s, Cannot resolve %s\nstack: %s\ndependencies: %s",
                getCurrentEitherComponent(), key, utils.getStack(), dependencies));
        logger.w(
            "(sub)component: %s, Cannot resolve %s\nstack: %s\ndependencies: %s",
            getCurrentEitherComponent(), key, utils.getStack(), dependencies);
    ...
    if (scoped) {
      ...
      generateFieldIfNeededAndInitialize(ClassName.get(Object.class), getLockFieldName(key));
      generateScopedProvisionMethod(injectorBuilder, key);
    }
  }

  private void generateProvisionMethodForEitherComponent(BindingKey key) {
    generateProvisionMethodForThoseFromTopLevel(key);
  }

  protected final void generateProvisionMethodFromComponentDependency(BindingKey key) {
    generateProvisionMethodForThoseFromTopLevel(key);
  }
   ...
   */
  protected final void generateProvisionMethodForThoseFromTopLevel(BindingKey key) {
    DependencyInfo dependencyInfo =
    ...
    switch (dependencySourceType) {
      ...
      case EITHER_COMPONENT:
        ...
        if (utils.isComponent(utils.getTypeElement(key))) {
          builder.append("this");
        }
        break;
      case EITHER_COMPONENT_BUILDER:
  ...
  protected final void generateProvisionMethodFromBindsInstance(BindingKey key) {
    generateProvisionMethodForThoseFromTopLevel(key);
  }

  protected final void generateProvisionMethodForBindsOptionalOf(BindingKey key, String suffix) {
  ...
  protected final void onProvisionMethodStart(MethodSpec.Builder methodSpecBuilder, BindingKey key) {
  ...
  protected final void onProvisionMethodEnd(MethodSpec.Builder methodSpecBuilder, BindingKey key) {
    if (!LOG_PROVISION_METHOD_ENABLED) {
      return;
    }
    methodSpecBuilder
  ...
  protected final void generateProvisionMethodFromModuleUniqueBinding(BindingKey key, String suffix) {
    // DependencyInfo dependencyInfo =
    //     Iterables.getOnlyElement(Utils.getDependencyInfosHandlingBox(dependencies, key));
   ...
   */
  protected final void generateProvisionMethodFromModuleBinding(
  ...
  protected final void generateGetProxyMethodIfNeeded(String packageString) {
  ...
  protected final void generateGetTypeMethod(ClassName proxyClassName) {
  ...
  protected final void generateFieldIfNeededAndInitialize(TypeName typeName, String fieldName) {
    generateFieldIfNeededCommon(typeName, fieldName, true, "");
  }

  protected final void generateFieldIfNeeded(TypeName typeName, String fieldName) {
    generateFieldIfNeededCommon(typeName, fieldName, false, "");
  }

  protected final void generateFieldIfNeeded(
      TypeName typeName, String fieldName, String fmt, Object... args) {
    generateFieldIfNeededCommon(typeName, fieldName, true, fmt, args);
  }

  protected final void generateFieldIfNeededCommon(
      TypeName typeName, String fieldName, boolean initialize, String fmt, Object... args) {
    if (!fieldsGenerated.add(fieldName)) {
      return;
    }
    FieldSpec.Builder fieldBuilder = FieldSpec.builder(typeName, fieldName, Modifier.PRIVATE);
    if (initialize) {
      if (fmt.isEmpty()) {
        fieldBuilder.initializer("new $T()", typeName);
      } else {
        fieldBuilder.initializer(fmt, args);
      }
    }
    injectorBuilder.addField(fieldBuilder.build());
  }

  protected final void generateGetTypeMethodIfNeeded(TypeElement module) {
    generateGetTypeMethodIfNeeded(ClassName.get(module));
  }

  protected final void generateGetTypeMethodIfNeeded(ClassName className) {
  ...
  protected final void generateProvisionMethodForProviderOrLazy(BindingKey key, String suffix) {
    ...
    methodSpecBuilder.addStatement(
        ...
        getProvisionMethodName(elementKey));
  ...
  protected final TypeSpec createAnonymousBuiltinTypeForUniqueBinding(BindingKey key) {
    return createAnonymousBuiltinType(key, null);
  }

  protected final TypeSpec createAnonymousBuiltinTypeForMultiBinding(
   ...
   */
  protected final TypeSpec createAnonymousBuiltinType(
    ...
    return TypeSpec.anonymousClassBuilder("")
        ...
        .addField(elementKey.getTypeName(), "var", Modifier.PRIVATE)
  ...
  protected final void generateScopedProvisionMethod(Builder componentSpecBuilder, BindingKey key) {
    ...
    builder
        ...
        .beginControlFlow("synchronized($L)", getLockFieldName(key))
  ...
  protected final String getFieldName(BindingKey key) {
    return Utils.getSourceCodeNameHandlingBox(key, dependencies);
  }

  protected final String getLockFieldName(BindingKey key) {
    return "lock_" + Utils.getSourceCodeNameHandlingBox(key, dependencies);
  }
  ...
  protected final void addNewStatementToMethodSpecByModuleOrCtor(
      MethodSpec.Builder methodSpecBuilder, DependencyInfo dependencyInfo, String newVarName) {
  ...
  protected final void generateMapContributors(
  ...
  protected final void generateInjectionMethod(BindingKey key) {
    generateInjectionMethod(utils.getClassFromKey(key), "inject");
  }

  protected final void generateInjectionMethod(TypeElement cls, String methodName) {

    // logger.n("cls: %s, injector: %s, method: %s", cls,
    // packagedInjectorClassName, methodName));
    if (!injectionMethodsDone.add(Pair.of(methodName, TypeName.get(types.erasure(cls.asType()))))) {
  ...
  protected final void addCallingProvisionMethod(StringBuilder stringBuilder, BindingKey key) {
    generateProvisionMethodIfNeeded(key);
  ...
  protected final void generateProvisionMethodFromClass(BindingKey key, String suffix) {
    ...
    if (shouldInjectAfterCreation() && !utils.isGenericNotSpecialized(cls.asType())
  ...
  /**
   * True for package injector and false for hub ones.
   */
  ...
  protected final void generateProvisionMethodForEitherComponentBuilder(BindingKey key) {
    // The top level injector must be the one that provides the wanted builder.
    generateProvisionMethodForThoseFromTopLevel(key);
  }

  protected final void generateProvisionMethodAndAppendAsParameter(
      BindingKey key, StringBuilder builder) {
    builder.append(generateProvisionMethodAndReturnCallingString(key)).append(", ");
  }

  protected final String generateProvisionMethodAndReturnCallingString(BindingKey key) {
    generateProvisionMethodIfNeeded(key);
    return getProvisionMethodName(key) + "()";
  }

  protected final boolean isEitherComponentBuilderProvsionMethodProvidedByModule(Element method) {
   ...
   * (sub)component.
   */
  protected final void generateImplicitProvisionMethodForEitherComponentBuilder(
  ...
  protected final ClassName getClassNameForEitherComponentBuilder(TypeElement componentBuilder) {
    TypeElement enclosingElement = (TypeElement) componentBuilder.getEnclosingElement();
  ...
  protected final void generateGetSubcomponentMethod(ExecutableElement method, Builder injectorBuilder) {
  ...
  protected final void generateExplicitProvisionMethodForEitherComponentBuilder(
  ...
  protected final boolean isInjectorOfScope(ClassName injectorClassName, TypeElement scope) {
  ...
  private String getCurrentEitherComponent() {
    return getPackageString() + "." + getInjectorSimpleName();
  }
}
